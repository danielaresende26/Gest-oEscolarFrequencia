...
exports.gerarDadosDashboard = async (escola_id) => {
  ...
  try {
    ...
    // Buscar todos os alunos da escola com suas turmas (IMPORTANTE: incluir o 'id' da turma no select)
    const { data: alunos, error: alunosErr } = await supabase
      .from('alunos')
      .select('id, nome, turmas!inner(id, nome, escola_id)')
      .eq('turmas.escola_id', escola_id);

    if (alunosErr) throw alunosErr;

    // Se não houver alunos, retornamos zerado sem erros
    if (!alunos || alunos.length === 0) {
      return { totalTurmas, alunosRisco: [], diasCriticos: [] };
    }

    const turmaIds = [...new Set(alunos.map(a => a.turmas.id))];
    ...
    (faltas || []).forEach(f => {
      const d = new Date(f.calendario_aulas.data + "T12:00:00");
    ...
    return {
      totalTurmas,
      alunosRisco,
      diasCriticos: estatisticasDias.filter(d => d.diaSemana !== 'Domingo' && d.diaSemana !== 'Sábado')
    };

  } catch (err) {
    console.error("❌ Erro em gerarDadosDashboard:", err.message);
    throw new Error("Erro ao montar dashboard: " + err.message);
  }
};
